...
{
    public static TimeLimit1 timeLimit;
    ...
    public static TimeLimit1 getLimits() {
        if (TimeLimit1.timeLimit == null) {
            initial();
        }
        return TimeLimit1.timeLimit;
    }
    
    public static void initial() {
        TimeLimit1.timeLimit = new TimeLimit1();
        ...
        for (int i = 0; i < vs.length; ++i) {
            ...
            if (!limit.getType().equals("SVIP")) {
                ...
                TimeLimit1.timeLimit.limits.add(limit);
            }
        }
        TimeLimit1.timeLimit.Sorting();
    }
    ...
    public void addLimit(String name, String type, String skin, String value, long time) {
        ...
        TimeLimit1.timeLimit.Sorting();
        if (limit.getType().equals("变身卡") || limit.getType().equals("强法型") || limit.getType().equals("加抗型") || limit.getType().equals("增益型") || limit.getType().equals("VIP") || limit.getType().equals("帮派") || limit.getType().equals("单人竞技场")) {
    ...
    public static void Timeout() {
        ...
        for (int i = TimeLimit1.timeLimit.limits.size() - 1; i >= 0; --i) {
            Limit limit = (Limit)TimeLimit1.timeLimit.limits.get(i);
            if (limit.getTime() != 0L && Util.getTime() > limit.getTime()) {
                ...
                TimeLimit1.timeLimit.removeLimit(limit);
            }
        }
        if (buffer != null) {
    ...
    public static JLabel getTimeJlabel() {
        if (TimeLimit1.image == null) {
            TimeLimit1.image = SkillTx.getImage();
        }
        JLabel jLabel = new JLabel() {
            @Override
            protected void paintComponent(Graphics g) {
                g.drawImage(TimeLimit1.image, 0, 0, 22, 22, null);
                g.translate(1, 1);
