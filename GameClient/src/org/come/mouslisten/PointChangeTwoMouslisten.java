...
public class PointChangeTwoMouslisten implements MouseListener
{
    private int type;
    
    public PointChangeTwoMouslisten(int type) {
        this.type = type;
    }
    ...
    public void rolepoint(RoleAttribute roleAttribute, int type, int point) {
        ...
        point = this.addpoint(this.getdz(type, roleAttribute), jpanel.getdian2(type), jpanel.getdian2(5), point);
        if (point == 0) {
            ZhuFrame.getZhuJpanel().addPrompt2("已无法改变点数");
        }
        else {
            jpanel.adddian2(type, point);
        }
    }
    
    public PointChangeTwoMouslisten() {
    }
    ...
    public int getdz(int type, RoleAttribute roleAttribute) {
        if (roleAttribute != null) {
            if (type == 0) {
                type = (int)roleAttribute.getBONETWO();
            }
            else if (type == 1) {
                type = (int)roleAttribute.getSPIRTWO();
            }
            else if (type == 2) {
                type = (int)roleAttribute.getPOWERTWO();
            }
            else if (type == 3) {
                type = (int)roleAttribute.getSPEEDTWO();
            }
            else if (type == 4) {
                type = (int)roleAttribute.getCALMTWO();
            }
        }
        return type;
    }
