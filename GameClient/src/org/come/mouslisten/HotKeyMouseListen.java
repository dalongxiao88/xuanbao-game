...
public class HotKeyMouseListen implements MouseListener
{
    public static final String F1 = "hot_0";
    public static final String F2 = "hot_1";
    public static final String F3 = "hot_2";
    public static final String F4 = "hot_3";
    public static final String F5 = "hot_4";
    public static final String F6 = "hot_5";
    public static final String F7 = "hot_6";
    public static final String F8 = "hot_7";
    public static Boolean isHide;
    public static int goodPlace;
    public static Map<String, String> keyMap;
    public static Goodstable goodstable;
    public static int replace;
    public static int replacepath;
    AthChartJframe athChartJframe;
    public ZhuJpanel zhuJpanel;
    public static GoodsResultArrBean goodarr;
    
    public HotKeyMouseListen(int goodPlace, ZhuJpanel zhuJpanel) {
        HotKeyMouseListen.goodPlace = goodPlace;
    ...
    public void mousePressed(MouseEvent e) {
        ...
        if (GoodsMouslisten.replacepath != -1) {
            Goodstable getgood = GoodsListFromServerUntil.Getgood(GoodsMouslisten.replacepath);
            System.out.println(getgood.getUsetime());
            System.out.println("GoodsMouslisten.goodPlace = " + getgood.getGoodsname());
            if ((long)getgood.getType() == 0L) {
                jLabel.setIcon(new ImageIcon(new ImageIcon("img/item/" + getgood.getSkin() + ".png").getImage().getScaledInstance(35, 35, 1)));
                HotKeyMouseListen.keyMap.put(jLabel.getName(), getgood.getGoodsid().toString());
                String skey = JSONUtil.toJsonStr(HotKeyMouseListen.keyMap);
                Util.writeHotkey(roleId, skey);
            }
        }
        if (GameJpanel.skill != null) {
            System.out.println("GoodsMouslisten.goodPlace = " + GameJpanel.skill.getSkillname());
            jLabel.setIcon(new ImageIcon(new ImageIcon("img/fighting-skill/" + GameJpanel.skill.getSkillid() + ".png").getImage().getScaledInstance(35, 35, 1)));
            HotKeyMouseListen.keyMap.put(jLabel.getName(), "s_" + GameJpanel.skill.getSkillid());
            String skey2 = JSONUtil.toJsonStr(HotKeyMouseListen.keyMap);
            Util.writeHotkey(roleId, skey2);
        }
        String gid = (String)HotKeyMouseListen.keyMap.get(jLabel.getName());
        if (StrUtil.isNotBlank(gid)) {
            if (NumberUtil.isNumber(gid)) {
                Goodstable goodstable = GoodsListFromServerUntil.chaxunsNew(Integer.parseInt(gid));
                if (e.getButton() == 3) {
                    ...
                    if (HotKeyMouseListen.replace != -1) {
                        goodreplace(-1, -1);
                    }
                    else if ((long)goodstable.getType() == 0L) {
                        doUse(goodstable);
                    }
                }
            }
            ...
            if (e.getButton() == 3 && FightingMixDeal.State != 0 && 1 == FightingMixDeal.State) {
                String fashu = gid.split("_")[1];
                Skill skill = UserMessUntil.getSkillById(fashu);
                if (skill != null) {
                    if (skill.getSkillname() != null&&skill.getSkillname().equals("度厄")) {
                        ...
                        FightingMonitor.mousesname = skill.getSkillname();
    ...
    private static void doUse(Goodstable goodstable) {
        if (goodstable != null) {
            ...
            else if (ZhuJpanel.getUseGoodsType() == 0) {
                try {
    ...
    public static void doHotKey(String key) {
        String gid = (String)HotKeyMouseListen.keyMap.get(key);
        if (StrUtil.isBlank(gid)) {
            return;
        }
        boolean number = NumberUtil.isNumber(gid);
        if (number) {
            Goodstable chaxuns = GoodsListFromServerUntil.chaxunsNew(Integer.parseInt(gid));
            if (chaxuns != null && (long)chaxuns.getType() == 0L) {
                doUse(chaxuns);
            }
        }
        else if (FightingMixDeal.State != 0 && 1 == FightingMixDeal.State) {
            String fashu = gid.split("_")[1];
            Skill skill = UserMessUntil.getSkillById(fashu);
            if (skill != null) {
                if (skill.getSkillname() != null&&skill.getSkillname().equals("度厄")) {
    ...
    public static void ManNoCombat(Goodstable goodstable, long type) throws Exception {
        HotKeyMouseListen.goodPlace = GoodsListFromServerUntil.getGoodsPlace(goodstable);
        ...
        if (EquipmentType != -1) {
            ...
            Lingbaogood.fushi(goodstable, HotKeyMouseListen.goodPlace);
        }
        ...
        if ((int)goodstable.getUsetime() <= 0) {
            GoodsListFromServerUntil.Delete(HotKeyMouseListen.goodPlace + GoodsListFromServerUntil.Pagenumber * 24);
        }
    }
    
    public static void UseEquipment(int EquipmentType, Goodstable goodstable) throws Exception {
        ...
        GoodsListFromServerUntil.mutualChange(EquipmentType, HotKeyMouseListen.goodPlace + GoodsListFromServerUntil.Pagenumber * 24);
    }
    ...
    public void mouseEntered(MouseEvent e) {
        ...
        boolean number = NumberUtil.isNumber(gid);
        if (number) {
            int chaxuns = GoodsListFromServerUntil.getGoodsPlaceById(new BigDecimal(gid));
            if (chaxuns != -1) {
                Goodstable goodstable = GoodsListFromServerUntil.Getgoodintotal(chaxuns);
                GoodsMsgJframe goodsMsgJframe = GoodsMsgJframe.getGoodsMsgJframe();
                int height = goodsMsgJframe.getGoodsMsgJpanel().showGood(goodstable);
                Point goodx = ZhuFrame.getZhuJpanel().framepoint(GameJpanel.getGameJpanel().getMousePosition(), goodsMsgJframe.getSize());
            ...
            String fashu = gid.split("_")[1];
            Skill skill = UserMessUntil.getSkillById(fashu);
            MsgJframe.getJframe().getJapnel().SM(skill, 25000.0, AnalysisString.lvlint((int)RoleData.getRoleData().getLoginResult().getGrade()));
        }
    }
    ...
    public int getGoodPlace() {
        return HotKeyMouseListen.goodPlace;
    }
    ...
    public static void goodreplace(int one, int two) {
        ...
        HotKeyMouseListen.replace = -1;
        HotKeyMouseListen.replacepath = -1;
    }
    
    public static void gooduse(Goodstable goodstable, int type) {
        HotKeyMouseListen.goodarr.setI(type);
        HotKeyMouseListen.goodarr.getList().clear();
        HotKeyMouseListen.goodarr.getList().add(goodstable);
        String sendmes = Agreement.getAgreement().packchangeAgreement(GsonUtil.getGsonUtil().getgson().toJson(HotKeyMouseListen.goodarr));
        SendMessageUntil.toServer(sendmes);
    }
    ...
    public static void gooduse(Goodstable good1, Goodstable good2, int type) {
        ...
        HotKeyMouseListen.goodarr.getList().clear();
        if (good1 != null) {
            HotKeyMouseListen.goodarr.getList().add(good1);
        }
        if (good2 != null) {
            HotKeyMouseListen.goodarr.getList().add(good2);
        }
        String sendmes = Agreement.getAgreement().packchangeAgreement(GsonUtil.getGsonUtil().getgson().toJson(HotKeyMouseListen.goodarr));
        SendMessageUntil.toServer(sendmes);
    }
    ...
    public void petUseGoods(Goodstable goodstable, long type) {
        if (Goodtype.FightingMedicine(type)) {
            ...
            hyd(goodstable);
            if ((int)goodstable.getUsetime() <= 0) {
                GoodsListFromServerUntil.Deleted(HotKeyMouseListen.goodPlace + GoodsListFromServerUntil.Pagenumber * 24);
            }
        }
    ...
    public void useNedan() {
        ...
        Goodstable good = GoodsListFromServerUntil.Getgood(HotKeyMouseListen.goodPlace);
        if (good == null) {
            return;
        }
    ...
    public void useMountSkillCard(Mount mount) {
        ...
        mountSkill.setSkillname(GoodsListFromServerUntil.Getgood(HotKeyMouseListen.goodPlace).getGoodsname());
        ...
        if (mount.getMountskill().size() != 0) {
            ...
            if ((boolean)bool) {
                ...
                SendMessageUntil.toServer(sendmes);
                GoodsListFromServerUntil.Getgood(HotKeyMouseListen.goodPlace).goodxh(1);
                gooduse(GoodsListFromServerUntil.Getgood(HotKeyMouseListen.goodPlace), 1);
                if ((int)GoodsListFromServerUntil.Getgood(HotKeyMouseListen.goodPlace).getUsetime() <= 0) {
                    GoodsListFromServerUntil.Deleted(HotKeyMouseListen.goodPlace + GoodsListFromServerUntil.Pagenumber * 24);
                }
    ...
    public static void usePsyVou() {
        QuackGameJframe.getQuackGameJframe().getGameJpanel().kyNum = (int)GoodsListFromServerUntil.Getgood(HotKeyMouseListen.goodPlace).getUsetime();
        String sendMes = Agreement.getFiveMsgAgreement("G");
        SendMessageUntil.toServer(sendMes);
    }
    
    public static void userRole(Goodstable good) {
        Limit limit = new Limit();
        long type = (long)good.getType();
        if (type == 100L || type == 888L || type == 2041L || type == 935L || type == 936L || type == 2126L) {
    ...
    public void userPet(Goodstable good, RoleSummoning pet) {
        ...
        if (type == 715L) {
            ...
            if (type == 2043L) {
                ...
                if (petid != 200123 && petid != 200116 && petid != 200117 && petid != 200097 && petid != 200098 && petid != 200099 && petid != 200100 && petid != 200101) {
                    String msg = "白泽 年?画中仙?颜如玉?垂云叟?五叶?范式之魂才能使用化形丹";
    ...
    public static void useFly(Goodstable good, Fly fly) {
        ...
        if (type == 1101L) {
            ...
            if (type == 1002L) {
                if (lvl != 100) {
                    ZhuFrame.getZhuJpanel().addPrompt("飞行器" + fly.getFlyname() + "等级不足");
                }
                return;
            }
        }
    ...
    static {
        ...
        HotKeyMouseListen.keyMap = new HashMap<>();
        HotKeyMouseListen.replace = -1;
        HotKeyMouseListen.replacepath = -1;
