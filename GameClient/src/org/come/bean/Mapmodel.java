...
public class Mapmodel
{
    ...
    public Mapmodel(double map_Screen_x, double map_Screen_y, JMap jMap, Gamemap gamemap) {
        this.doors = new ArrayList<>();
        this.point = new PathPoint(0, 0);
    ...
    public byte[][] getNodes() {
        if (this.Nodes == null || this.Nodes.length != this.jMap.getMaprules().length || this.Nodes[0].length != this.jMap.getMaprules()[0].length) {
            ...
            for (int i = 0; i < this.Nodes.length; ++i) {
                for (int k = 0; k < this.Nodes[i].length; ++k) {
                    this.Nodes[i][k] = 0;
                }
            }
        }
    ...
    public void setShot_x(int shot_x) {
        ...
        if (Math.abs(this.shot_x - shot_x) > Mapmodel.LIMIT_X) {
            this.shot_x = shot_x + ((shot_x - this.shot_x < 0) ? Mapmodel.LIMIT_X : (-Mapmodel.LIMIT_X));
        }
    ...
    public void setShot_y(int shot_y) {
        ...
        if (Math.abs(this.shot_y - shot_y) > Mapmodel.LIMIT_Y) {
            this.shot_y = shot_y + ((shot_y - this.shot_y < 0) ? Mapmodel.LIMIT_Y : (-Mapmodel.LIMIT_Y));
        }
