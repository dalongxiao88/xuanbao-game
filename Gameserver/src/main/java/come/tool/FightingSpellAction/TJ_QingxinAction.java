...
public class TJ_QingxinAction implements SpellAction
{
    @Override
    public void spellAction(ManData manData, FightingSkill skill, FightingEvents events, Battlefield battlefield) {
        AddState state1 = manData.xzstate(TypeUtil.TZ_BDBQ);
        long Hurt = (long)(skill.getSkillhurt() / 2.0 * (double)manData.getAp());
        double mzjc = 0.0;
        double dsjc = 0.0;
        ManData child = battlefield.getChild(manData);
        FightingSkill skill2 = manData.getAppendSkill(1261);
        if (skill2 != null) {
        ...
        for (int i = 0; i < nodatas.size(); ++i) {
            ...
            if (nomyData.getStates() == 0) {
                ...
                if ((double)(Battlefield.random.nextInt(100) + 1) > nomyData.getsx(4, TypeUtil.SX_SBL) + dsjc - manData.getQuality().getRolefmzl() - mzjc - manData.mz) {
                    ...
                    if (state1 != null) {
                        List<ManData> datas = MixDeal.getdaji(7, manData.getCamp(), events, battlefield);
