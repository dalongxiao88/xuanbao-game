...
public class TJ_DNTGAction implements SpellAction
{
    ...
    public void spellAction(ManData manData, FightingSkill skill, FightingEvents events, Battlefield battlefield) {
        ...
        if (!PK_MixDeal.isPK(battlefield.BattleType) && (manData.getModel().startsWith("999999") || manData.getModel().startsWith("5555559"))) {
            Zap = (int)((double)Zap * 0.2);
        }
        for (int k = nomyDatas.size() - 1; k >= 0; --k) {
            ManData data2 = (ManData)nomyDatas.get(k);
            int g = 0;
            int maxg = GMax(manData, data2, ljjc, battlefield);
            while (data2.getStates() == 0 && g < maxg) {
                ...
                FightingState move = MixDeal.skillmove(data2, manData, "11");
                zls2.add(move);
                if (k == 0 && zap != 0L && zzs != null) {
                    move.setText("合力一击#2");
                    for (int m = 0; m < zzs.size(); ++m) {
                        ManData zz = (ManData)zzs.get(m);
                        zls2.add(MixDeal.skillmove(data2, zz, "11"));
                    }
                }
            }
        }
