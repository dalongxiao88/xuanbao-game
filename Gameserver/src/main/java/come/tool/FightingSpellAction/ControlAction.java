            ...
            for (int i = 0; i < datas.size(); ++i) {
                ...
                if (Accepter != null) {
                    ...
                    if (!change.getChangetype().equals("")) {
                        if (skill.getSkilltype().equals("遗忘") && manData.getSkillId(30019) != null && Battlefield.isV(manData.getSkillId(30019).getSkillgain() + manData.getSkillId(30019).getS1())) {
                            data.addAddState("忘情石", 0.0D, 0.0D, 2);
                            Accepter.setEndState_1("忘情石");
                        }
                        if (data.getSkillId(30018) != null) {
                            battlefield.XB_ZhuChouSuo(manData, data, data.getSkillId(30018).getSkillgain1() + data.getSkillId(30018).getS2());
                        }
                        ++size;
                        if (data.getType() == 0) {
                    ...
                    } else if (data.getSkillId(30018) != null) {
                        battlefield.XB_ZhuChouSuo(manData, data, data.getSkillId(30018).getSkillgain1());
                    }
//                if (i == 0) {
//                    battlefield.FightingBox(manData, data, skill, Math.abs(change.getChangehp()),false);
//                }
                    hurt = change.getChangehp();
                ...
                if (skill.getSkilltype().equals("中毒")) {
                    double p = 0.0D;
                    if (data.getStates() != 0 && manData.getSkillId(30029) != null) {
                        p = manData.getSkillId(30029).getS1();
                    }
                    boolean is = true;
                    for (ManData data1 : battlefield.fightingdata) {
                        if (data1.getCamp() == manData.getCamp() && data1.getMan() != manData.getMan() && data1.getSkillId(30029) != null) {
                            is = false;
                        }
                    }
                    if (is && manData.getSkillId(30029) != null) {
                        p = manData.getSkillId(30029).getP1();
                    }
                    if (manData.getSkillId(30029) != null && Battlefield.isV(manData.getSkillId(30029).getSkillgain() + p)) {
                        ...
                        int hurt1 = (int) Math.min(-hurt * (manData.getSkillId(30029).getSkillgain1() + manData.getSkillId(30029).getE1()) / 100.0D, manData.getHp_z() * manData.getSkillId(30029).getSkillhurt() / 100.0D);
                        changeFighting1.setChangehp(hurt1);
    ...
    public static List<FightingSkill> getL_FY_Skills(ManData myData, FightingSkill skill, int type) {
        ...
        skills = addSkill(myData.getSkillType(TypeUtil.TY_L_10157), skills);
    ...
    public static List<FightingSkill> getSkills(ManData myData, FightingSkill skill, int type) {
        ...
        if (skill.getSkilltype().equals("封印")) {
            ...
            if (PK_MixDeal.isPK(type)) {
                ...
                skills = addSkill(myData.getSkillType(TypeUtil.TY_G_10084), skills);
                skills = addSkill(myData.getSkillType(TypeUtil.TY_G_10089), skills);
                skills = addSkill(myData.getSkillType(TypeUtil.TY_G_10104), skills);
            } else {
            ...
            if (PK_MixDeal.isPK(type)) {
                ...
                skills = addSkill(myData.getSkillType(TypeUtil.TY_G_10084), skills);
                skills = addSkill(myData.getSkillType(TypeUtil.TY_G_10094), skills);
            }
            ...
            if (PK_MixDeal.isPK(type)) {
                ...
                skills = addSkill(myData.getSkillType(TypeUtil.TY_G_10099), skills);
                skills = addSkill(myData.getSkillType(TypeUtil.TY_G_10104), skills);
            }
        ...
        } else if (skill.getSkilltype().equals("三尸虫") && PK_MixDeal.isPK(type)) {
            skills = addSkill(myData.getSkillType(TypeUtil.TY_L_DZXY), skills);
        }
        return skills;
    }
    ...
    public ChangeFighting TypeProbabilityNoHurt(ManData myData, ManData data, FightingSkill skill, List<FightingSkill> skills, Battlefield battlefield) {
        ...
        double addQF = 0.0;
        double addKX = 0.0;
        if (PK_MixDeal.isPK(battlefield.BattleType) && skill.getSkilltype().equals("遗忘")) {
            List<FightingSkill> skillList = myData.getSkillTypes(Arrays.asList(new String[]{TypeUtil.TY_G_10085, TypeUtil.TY_G_10095, TypeUtil.TY_G_10100}));
            for (FightingSkill fightingSkill : skillList) {
                if (data.getType() == 0 && !battlefield.hasPet(data)) {
                    addHS += Battlefield.getSkillhurt(fightingSkill, "忘");
                }
            }
        }
        if (Calculation.getCalculation().renfa(myData, data, skill.getSkillhitrate(), type, addHS, addQF, addKX)) {
            if (data.xzstate("玄鳞甲") != null && data.xzstate("玄鳞甲").getStateEffect() > 0.0D) {
                data.xzstate("玄鳞甲").setStateEffect(data.xzstate("玄鳞甲").getStateEffect() - 1.0D);
                if (data.xzstate("玄鳞甲").getStateEffect() == 0.0D) {
                    FightingState state = new FightingState();
                    state.setMan(data.getMan());
                    state.setCamp(data.getCamp());
                    state.setEndState_2("玄鳞甲");
                    data.RemoveAbnormal("玄鳞甲");
            ...
            changeFighting.setSkill(skill);
            if (skills != null) {
                for (int i = 0; i < skills.size(); ++i) {
                    FightingSkill fightingSkill2 = (FightingSkill) skills.get(i);
                    int id = fightingSkill2.getSkillid();
                    if (id == 6034) {
                        changeFighting.setChangevlaue2(changeFighting.getChangevlaue2() + fightingSkill2.getSkillhurt());
                    } else if (id == 9341) {
                        if (skill.getSkillid() == 1074) {
                            changeFighting.setChangevlaue2(changeFighting.getChangevlaue2() + fightingSkill2.getSkillhurt());
                        }
                    } else if (id == 9101 || id == 9141) {
                        if (Battlefield.isV(fightingSkill2.getSkillhurt())) {
                        ...
                        if (data.xzstate("混乱") != null) {
                            changeFighting.setChangehp(-(int) ((double) data.getHp_z() * fightingSkill2.getSkillhurt() / 100.0));
                        }
                    } else if (id == 9343) {
                        if (data.getType() == 1 && Battlefield.isV(fightingSkill2.getSkillhurt())) {
                            changeFighting.setSkill(fightingSkill2);
                        }
                    } else if (id == 9351) {
                    ...
                    } else if (id == 9112 || id == 9125 || id == 9127 || id == 9128 || id == 9131 || id == 9150 || id == 9882) {
                        if (Battlefield.isV(fightingSkill2.getSkillhurt())) {
                            changeFighting.setSkill(fightingSkill2);
                        }
                    } else if (id == 10052) {
                        if (data.xzstate("遗忘") != null) {
                            changeFighting.setChangemp((int) ((double) (-data.getMp()) * fightingSkill2.getSkillhurt() / 100.0));
                        }
                    } else if (id == 10053) {
                        if (data.xzstate("遗忘") != null) {
                            data.setHuoyue(data.getHuoyue() - data.getHuoyue() * fightingSkill2.getSkillhurt() / 100.0);
                        }
                    } else if (id == 10054) {
                        if (data.xzstate("遗忘") != null && Battlefield.isV(fightingSkill2.getSkillhurt())) {
            ...
            if (type.equals("封印") &&
                    myData.getSkillId(30031) != null) {
                if (data.xzstate("凝霜印") != null) {
                    data.xzstate("凝霜印").setStateEffect5(data.xzstate("凝霜印").getStateEffect5() + 1.0D);
                    FightingSkill fightingSkill = myData.getSkillId(30031);
                    if (myData.xxywl < fightingSkill.getE3()) {
                        if (data.getType() == 0) {
                            if (Battlefield.isV(fightingSkill.getE1())) {
                                myData.xxywl = (int) (myData.xxywl + fightingSkill.getSkillhitrate());
                            }
                        } else if (Battlefield.isV(fightingSkill.getE2())) {
                            myData.xxywl = (int) (myData.xxywl + fightingSkill.getSkillhitrate());
                        }
                    }
                    if (myData.xxyfs < fightingSkill.getP3()) {
                        if (data.getType() == 0) {
                            if (Battlefield.isV(fightingSkill.getP1())) {
                                myData.xxyfs = (int) (myData.xxyfs + fightingSkill.getSkillgain());
                            }
                        } else if (Battlefield.isV(fightingSkill.getP2())) {
                            myData.xxyfs = (int) (myData.xxyfs + fightingSkill.getSkillgain());
                        }
                    }
                } else {
                    ...
                    fightingState.setEndState_1("凝霜印");
                    data.addAddState("凝霜印", 999, fightingSkill.getSkillhitrate(), fightingSkill.getSkillhurt(), fightingSkill.getSkillgain(), fightingSkill.getSkillgain1(), 1.0D, fightingSkill.getS1());
                    if (myData.xxywl < fightingSkill.getE3()) {
                        if (data.getType() == 0) {
                            if (Battlefield.isV(fightingSkill.getE1())) {
                                myData.xxywl = (int) (myData.xxywl + fightingSkill.getSkillhitrate());
                ...
                myData.getSkillId(30028) != null && Battlefield.isV(myData.getSkillId(30028).getSkillgain() + myData.getSkillId(30028).getS1())) {
            if (myData.xzstate("摄魂铃") != null) {
                myData.xzstate("摄魂铃").setStateEffect(Math.min(myData.getSkillId(30028).getSkillcontinued() + myData.getSkillId(30028).getE1(), myData.xzstate("摄魂铃").getStateEffect() + 1.0D));
            } else {
                List<FightingState> states = new ArrayList<>();
                FightingState fightingState = new FightingState();
                fightingState.setCamp(myData.getCamp());
                fightingState.setMan(myData.getMan());
                fightingState.setEndState_1("摄魂铃");
                myData.addAddState("摄魂铃", (1 + ((myData.getSkillId(30028).getSkillgain() + myData.getSkillId(30028).getS1() > 100.0D) ? 1 : 0)), 1.0D, 3 + (Battlefield.isV(myData.getSkillId(30028).getP1()) ? 1 : 0));
                states.add(fightingState);
                FightingEvents events = new FightingEvents();
                events.setAccepterlist(states);
                battlefield.NewEvents.add(events);
        ...
        if (type.equals("中毒")) {
            ...
            if (skills != null) {
                for (int j = 0; j < skills.size(); ++j) {
                    FightingSkill fightingSkill3 = (FightingSkill) skills.get(j);
                    int id2 = fightingSkill3.getSkillid();
                    if (id2 == 6037) {
                        upxs += fightingSkill3.getSkillhurt() / 100.0;
                        changeFighting.setChangesum(1);
                    } else if (id2 == 9161) {
                        qds += fightingSkill3.getSkillhurt();
                    } else if (id2 == 9162 || id2 == 9163) {
                        qds += fightingSkill3.getSkillhurt();
                    } else if (id2 == 9164) {
                        data.addAddState(fightingSkill3.getSkilltype(), fightingSkill3.getSkillhurt(), 0.0, battlefield.CurrentRound + 1);
                    } else if (id2 == 9165) {
                        if (data.getType() == 1) {
                            qds += fightingSkill3.getSkillhurt();
                        }
                        ...
                        qds += fightingSkill3.getSkillhurt();
                        hurtxs -= fightingSkill3.getSkillhurt() / 100.0;
            ...
            int hurt = Math.toIntExact((long) Calculation.getCalculation().getzdsh(myData, data, skill, qds));
            up = (int) ((double) up * upxs);
            hurt = (int) ((double) hurt * hurtxs);

            boolean zxsg = false;

//            if (PK_MixDeal.isPK(battlefield.BattleType)) {//这个是不是PK生效
            addState = myData.xzstate(TypeUtil.TZ_ZXSG);//钻心蚀骨套装
            if (addState != null) {
                AddState addState22 = data.xzstate(TypeUtil.TZ_HURT2_ZXSG);
                if (addState22 != null && myData.getId() == addState22.getStateEffect2()) {
                    hurt = (int) (hurt * addState22.getStateEffect());
                    up += (int) (hurt * addState22.getStateEffect() * 0.67);
                    addState22.setSurplus(addState22.getSurplus() + 1);
                } else {
                    addState22 = data.xzstate(TypeUtil.TZ_HURT1_ZXSG);
                    if (addState22 != null && myData.getId() == addState22.getStateEffect2()) {
                        hurt = (int) (hurt * addState22.getStateEffect());
                        up += (int) (hurt * addState22.getStateEffect() * 0.67);
                        addState22.setStateEffect(addState22.getStateEffect() * 1.50);
                        data.addAddState2(TypeUtil.TZ_HURT2_ZXSG, addState22.getStateEffect(), myData.getId(), 2);
                    } else {
                        data.addAddState2(TypeUtil.TZ_HURT1_ZXSG, addState.getStateEffect(), myData.getId(), 2);
                    }
                }
                zxsg = true;
            }
//            }
            ...
            FightingSkill fightingSkill4 = myData.getSkillId(Integer.parseInt(TypeUtil.TJ_MTL));
            if (fightingSkill4 != null && PK_MixDeal.isPK(battlefield.BattleType)) {
//                changeFighting.setChangemp((int)((double)changeFighting.getChangehp() * 0.15));
//                data.addAddState(TypeUtil.TJ_MTL, 0.15, 0.0, hurt / 2);
                changeFighting.setChangemp(-(int) ((double) data.getMp_z() * 0.40));
                data.addAddState(TypeUtil.TJ_MTL, 0.40, 0.0, data.getMp_z() / 2);
            }
...
//                data.addAddState(TypeUtil.TJ_FSSS, 0.2, 0.0, hurt / 2);
                changeFighting.setChangemp(-(int) ((double) data.getMp_z() * 0.45));
                data.addAddState(TypeUtil.TJ_FSSS, 0.45, 0.0, data.getMp_z() / 2);

            }
            changeFighting.setChangevlaue((double) (hurt / 2));
            if (zxsg) {
                changeFighting.setChangevlaue(changeFighting.getChangevlaue() / 2);
            }
        }
