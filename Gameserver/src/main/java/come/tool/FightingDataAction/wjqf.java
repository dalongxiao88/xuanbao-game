...
public class wjqf implements DataAction
{
    @Override
    public void analysisAction(ManData manData, FightingEvents fightingEvents, String type, Battlefield battlefield) {
        boolean isHSSF = TypeUtil.TZ_CXYF.equals(type);
        type = TypeUtil.TZ_CXYF;
        FightingSkill fightingSkill = manData.getAttacks("万箭齐发").getSkill();
        ...
        gjz.setText("看我的快刀斩乱麻#24");
        gjz.setSkillskin("cgyn");
        gjz.setSkillskin("1236");
        list.add(gjz);
        ...
        MixDeal.skillmove(nomyDatas, battlefield, manData, "attack");
        manData.getQuality().setRolefmzl(manData.getQuality().getRolefmzl() + 200.0);
        manData.getQuality().setRolefkbl(manData.getQuality().getRolefkbl() + 100.0);
        manData.getQuality().setRolefzml(manData.getQuality().getRolefzml() + 100.0);
        manData.setAp((int)Math.ceil((double)(manData.getAp() * 20)));
        long ap = 0L;
        long zap = 0L;
        long zap2 = 0L;
        long zap3 = 0L;
        ...
        for (int l = nomyDatas.size() - 1; l >= 0; --l) {
            ...
            int maxg = 1 + (((double)Battlefield.random.nextInt(100) < ljl) ? ljv : 2);
            if (PK_MixDeal.isPK(battlefield.BattleType)) {
            ...
            while (data3.getStates() == 0 && g < maxg) {
                ...
                if (g == 1) {
                    ace.setSkillskin("30001");
                }
                else {
                    ace.setSkillskin((g % 3 == 1) ? "30000" : ((g % 3 == 2) ? "30000" : "30000"));
                }
