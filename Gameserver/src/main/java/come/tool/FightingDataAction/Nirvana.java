...
public class Nirvana implements DataAction
{
    ...
    public void analysisAction(ManData manData, FightingEvents fightingEvents, String type, Battlefield battlefield) {
        ...
        if (skill.getSkilltype().equals(TypeUtil.BB_JR)) {
            FightingSkill skill3 = manData.getSkillType(TypeUtil.BB_E_DNBS);
            if (skill3 != null && Battlefield.isV(skill3.getSkillgain())) {
                text = "大难不死#2";
                changeFighting.setChangehp((int)((double)manData.getHp_z() * skill3.getSkillgain() / 100.0));
                changeFighting.setChangemp((int)((double)manData.getMp_z() * skill3.getSkillgain() / 100.0));
            }
            else {
        ...
        manData.executePrcc(idx, States);
