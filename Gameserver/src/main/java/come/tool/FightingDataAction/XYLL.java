...
public class XYLL implements DataAction
{
    @Override
    public void analysisAction(ManData manData, FightingEvents fightingEvents, String type, Battlefield battlefield) {
        ...
        if (!type.equals("混乱技")) {
            ...
            FightingSkill fightingSkill = manData.getSkillType(TypeUtil.BB_XYLL);
            ...
            FightingSkill skill_224 = null;
            FightingSkill skill_225 = null;
            FightingSkill skill_226 = null;
            FightingSkill skill_227 = null;
            FightingSkill skill_228 = null;
            FightingSkill skill_229 = null;
            for (int i = manData.getSkills().size() - 1; i >= 0; --i) {
                ...
                if (sl.getSkillbeidong() == 1) {
                    ...
                    else if (sl.getSkilltype().equals(TypeUtil.BB_XYLL)) {
                        skill_229 = sl;
                    }
                }
            }
            List<ManData> zzs = null;
            if (skill_224 != null && Battlefield.isV(skill_224.getSkillgain())) {
                zzs = new ArrayList<>();
            ...
            while (noman.getStates() == 0 && g < maxg) {
                ...
                ace.setSkillskin("1290");
                if (skill_227 != null && Battlefield.isV(skill_227.getSkillgain())) {
                    ap += zap2;
                    gjz.setText("大圣附身#2");
                    ace.setSkillskin(TypeUtil.BB_E_DSFS);
                }
                else if (skill_228 != null && Battlefield.isV(skill_228.getSkillgain())) {
                    ap += zap3;
                    gjz.setText("乾坤一掷#2");
                ...
                if (skill_229 != null) {
                    if (skill_229.getSkillhurt() > 0.0) {
                        lastAP = (long)((double)lastAP + (double)(lastAP * 30L) / skill_229.getSkillhurt());
                    }
                    if (Battlefield.isV(skill_229.getSkillgain())) {
                        gjz.setStartState("特效1");
                        ace.setSkillskin("4016");
                        gjz.setText("仙音缕缕#2");
                        List<ManData> datas = battlefield.getZW2(noman);
                        for (int k = datas.size() - 5; k >= 0; --k) {
                ...
                if (skill_225 != null && Battlefield.isV(skill_225.getSkillgain()) && manData.yijibiming) {
                    manData.yijibiming = false;
                ...
                if (skill_226 != null && (isHSSF || Battlefield.isV(skill_226.getSkillgain() / 20.0))) {
                    ...
                    for (int m = datas2.size() - 1; m >= 0; --m) {
                        ...
                        if (nomyData3.getStates() == 0) {
                            ...
                            ap = (long)PhyAttack.Hurt((long)((double)manData.getAp() * skill_226.getSkillgain() / 100.0), g, manData, nomyData3, type, ace3, zls, battlefield, 10.0, 10.0);
            ...
            List<FightingState> listZhui2 = new ArrayList<>();
            for (int j2 = 0; j2 < gjEventss.size(); ++j2) {
                listZhui2.addAll(((FightingEvents)gjEventss.get(j2)).getAccepterlist());
            }
            FightingEvents fightingEvents2 = new FightingEvents();
            fightingEvents2.setAccepterlist(listZhui2);
