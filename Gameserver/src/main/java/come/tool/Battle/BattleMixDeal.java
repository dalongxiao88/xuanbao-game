...
public class BattleMixDeal
{
    ...
    public static BattleData initData(FightingForesee fightingForesee, long mapid) {
        ...
        battleData.setTeam2((fightingForesee.getErdui() != null && !fightingForesee.getErdui().equals("")) ? fightingForesee.getErdui().split("\\|") : BattleMixDeal.Teams);
        Robots robots = null;
        if (fightingForesee.getRobotid() != null) {
            robots = (Robots)GameServer.getAllRobot().get(fightingForesee.getRobotid());
        }
        ...
        if (robots != null && (battleData.getMonsterBean() == null || battleData.getMonsterBean().getFollow() == null)) {
    ...
    public static BattleData initData(MapMonsterBean bean, String[] teams, long mapid) {
        ...
        battleData.setTeam2(BattleMixDeal.Teams);
        battleData.setBattletime(System.currentTimeMillis());
        Robots robots = null;
        battleData.setMonsterBean(bean);
        robots = (Robots)GameServer.getAllRobot().get(bean.getRobotid() + "");
    ...
    public static BattleData initData(String[] teams, OneArenaRole myArenaRole, OneArenaRole otherArenaRole, long mapid) {
        ...
        Robots robots = (Robots)GameServer.getAllRobot().get("99999");
        battleData.setRobots(robots);
        battleData.getBattlefield().dropModel = robots.getDropModel();
        battleData.setBattletime(System.currentTimeMillis());
        return battleData;
    ...
    public static BattleData initWSSData(String[] teams, int wssType, long mapid) {
        ...
        Robots robots = (Robots)GameServer.getAllRobot().get(4000 + wssType + "");
        battleData.setRobots(robots);
        battleData.getBattlefield().dropModel = robots.getDropModel();
        battleData.setBattletime(System.currentTimeMillis());
        return battleData;
    ...
    public static String isLvl(int lvl, int[] lvls) {
        ...
        if ((manturn <= lvls[0] && manlvl < lvls[1]) || manlvl > lvls[3]) {
    ...
    static {
        BattleMixDeal.Teams = new String[0];
    }
}
