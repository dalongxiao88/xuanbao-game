...
public class PackRecord
{
    ...
    public int getPutTXKid() {
        ...
        while (i < txs.length) {
            if (txs[i].endsWith("&")) {
                id = Integer.parseInt(txs[i].substring(0, txs[i].length() - 1));
                break;
            }
            else {
                ++i;
    ...
    public PartJade setPartJade(int suitid, int partId, int pz, int sum) {
        ...
        if (suit == null || suit.equals("")) {
            ...
            PartJade partJade2 = new PartJade(suitid, partId);
                ...
                if (suitid != sid) {
                    ...
                    for (int k = 1; k < suits2.length; ++k) {
                        ...
                        if (partId != pid) {
                            ...
                            is2 = false;
                            partJade2.initJade(suits3);
                            partJade2.setJade(pz, sum);
                            if (!partJade2.isJade()) {
                                buffer2.append("-");
                                buffer2.append(partJade2.toString());
                            }
                        }
                    }
                    if (is2) {
    ...
    public String isCard(String id) {
        ...
        if (vs.length <= 2) {
            return this.lCard = this.lCard + "|" + id;
        }
        return this.lCard = id + "|" + vs[0] + "|" + vs[1];
    }
