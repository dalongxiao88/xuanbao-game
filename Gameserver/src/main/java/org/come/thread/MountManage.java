...
{
    private List<Mount> addList;
    private List<BigDecimal> delList;
    private List<Mount> updList;
        ...
        if (this.addList.size() > 100) {
            try {
                ...
                for (int i = this.addList.size() - 1; i >= 0; --i) {
                    try {
                        AllServiceUtil.getMountService().insertMountSingle((Mount)this.addList.get(i));
                    }
                    catch (Exception e2) {
                        try {
        ...
        if (this.delList.size() > 100) {
            try {
                ...
                for (int i = this.delList.size() - 1; i >= 0; --i) {
                    try {
                        AllServiceUtil.getMountService().deleteMountsByMidsql((BigDecimal)this.delList.get(i));
                    }
                    catch (Exception e2) {
                        try {
        ...
        if (this.updList.size() > 100) {
            try {
                ...
                for (int i = this.updList.size() - 1; i >= 0; --i) {
                    try {
                        AllServiceUtil.getMountService().updateMountsql((Mount)this.updList.get(i));
                    }
                    catch (Exception e2) {
                        try {
