...
public class GolemBean {
    ...
    private Double sp = 0.26;//速度
    private int bangtime=0;//机器人帮战进度 0-无 1-开启帮战
    private Long sleeptime;//帮战时等待时间
    private BigDecimal gangid;//帮派id
    private int type = 0;//帮战人物状态 0:空闲 1:战斗中 2:组好了队 3:点门 4:点建筑物
    private int SLtype = 0;//水陆状态 0:未参战 1:参战
    private int BWtype = 0;//水陆状态 0:未参战 1:参战
    private int QMtype = 0;//全民状态 0:  1:
...
//        RoleTable roleInfo = AllServiceUtil.getRoleTableService().selectRoleTableByRoleName(loginResult.getRolename());
//        if (roleInfo == null) {
//            return ;
//        }
...
//
//        List<LoginResult> allLogin = AllServiceUtil.getRoleTableService().selectRoleByUserid(userInfo.getUser_id(), BigDecimal.ZERO.subtract(userInfo.getUser_id()));
//        // 进入游戏
...
//    }
    ...
    /**
     * 距离
     * @param type 1 怪物 2 玩家
     **/
    public boolean nearby(int type) {
        if (type == 1) {
            if (target == null) {
                return false;
            }
...
//                return false;
//            }
//            String[] positions = Fight.getGuide().split("-");
//            int mapId = Integer.parseInt(positions[0]);
//            int x = Integer.parseInt(positions[1]);
//            int y = Integer.parseInt(positions[2]);
            return false;
...
//    }
    public double getSp() {
//        if (sp == 0.26) {
//            sp=this.loginResult.getRoleShow().getFlySpeed();
//        }
        return sp;
    }
    ...
    /** 飞行器操作 0装备 1飞行 2降落 **/
    public void flyAction(int tyep, int zs) {
        if (tyep == 0) {
...
//            }
            ...
            String fskin = jiarenFlySkin(zs);
            loginResult.setFly_id(1);
            loginResult.setFlyskin(fskin);
            loginResult.getRoleShow().setFly_id(1);
            loginResult.getRoleShow().setFlyskin(fskin);
            SendMessage.sendMessageToMapRoles(loginResult.getMapid(), Agreement.getAgreement().upRoleShowAgreement(GsonUtil.getGsonUtil().getgson().toJson(loginResult.getRoleShow())));
...
//            }
            ...
            loginResult.setFly_id(0);
            loginResult.setFlyskin(null);
            loginResult.getRoleShow().setFly_id(0);
            loginResult.getRoleShow().setFlyskin(null);
            SendMessage.sendMessageToMapRoles(loginResult.getMapid(), Agreement.getAgreement().upRoleShowAgreement(GsonUtil.getGsonUtil().getgson().toJson(loginResult.getRoleShow())));
        }
    }
...
//    }
    ...
    public boolean iswait() {
        if (sleeptime != null && sleeptime != 0) {
            if (sleeptime > System.currentTimeMillis()) return true;
            sleeptime=0L;
        }
        return false;
    }
    ...
    public BigDecimal getGangid() {
        if (gangid == null|| gangid.compareTo(BigDecimal.ZERO) == 0) {
            gangid = getLoginResult().getGang_id();
        }
        return gangid;
    }
    ...
    public int getBangtime() {
        return bangtime;
    }

    public void setBangtime(int bangtime) {
