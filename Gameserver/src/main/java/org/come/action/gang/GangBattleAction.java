...
public class GangBattleAction implements IAction
{
            ...
            if (message.startsWith("M")) {
                ...
                if (v[0].equals("keju")) {
                    if (!KejuAction.iskeju) {
                        KejuAction.iskeju = true;
                        KejuAction.role.clear();
                    }
                    if (KejuAction.role.size() != 0) {
                        for (int i = 0; i <= KejuAction.role.size() - 1; ++i) {
                            if (((String[])KejuAction.role.get(i))[0].equals(v[1])) {
                                SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("#R你已经领取了任务，请前往1号考官处答题"));
                                return;
                            }
                        }
                    }
                    ...
                    switch (zhi2) {
                        ...
                        case 9: {
                            this.cn(ctx, Integer.parseInt(v[1]));
                            break;
                        }
                        case 10: {
                            this.gjt(ctx, Integer.parseInt(v[1]));
                            break;
                        }
                        case 11: {
                            this.qx(ctx, Integer.parseInt(v[1]));
                            break;
                        }
                        case 12: {
                            this.qdph(ctx, Integer.parseInt(v[1]));
                            break;
                        }
                        case 13: {
                            this.bty(ctx);
                        ...
                        case 29:
                        case 30: {
                            this.LTS(zhi2, (v.length <= 1) ? 0 : Integer.parseInt(v[1]), ctx);
                            break;
                        }
                        case 31:
                        case 32: {
                        ...
                        case 35: {
                            this.FOLLOW(zhi2, Integer.parseInt(v[1]), ctx);
                            break;
                        }
                        case 39:
                        case 40: {
                        ...
                        case 48:
                        case 49: {
                            this.GangNpc(zhi2, (v.length > 1) ? v[1] : null, ctx);
                            break;
                        }
                        case 50: {
                            this.JRWSS(zhi2, ctx);
    ...
    public void CHIJI(int value,ChannelHandlerContext ctx){
        ...
        if (value==55) {//参加吃鸡
            String msg=cjScene.addEnroll(ctx, roleInfo);
            if (msg!=null) {
    ...
    public void WSS(ChannelHandlerContext ctx, int type) {
        ...
        BattleData battleData = (BattleData)BattleThreadPool.BattleDatas.get(Integer.valueOf(type));
        if (battleData != null) {
    ...
    private void JRWSS(int zhi1, ChannelHandlerContext ctx) {
        ...
        action.action(ctx, GangBattleAction.WSS);
    }
    ...
    public void intobang(ChannelHandlerContext ctx) {
        ...
        if (bangFight == null) {
            if (roleInfo.getGang_id().longValue() == 0L) {
            ...
            if (bangFight.addMember(roleInfo.getRolename(), roleInfo.getGang_id())) {
                ...
                IAction action = (IAction)ParamTool.ACTION_MAP.get("changemap");
                action.action(ctx, bangFight.iscamp(roleInfo.getGang_id()) ? GangBattleAction.HOME_Left : GangBattleAction.HOME_Right);
            }
        }
    }
    ...
    public void outbang(ChannelHandlerContext ctx) {
        ...
        IAction action = (IAction)ParamTool.ACTION_MAP.get("changemap");
        action.action(ctx, GangBattleAction.OUTBANG);
    }
    ...
    public void intomaster(ChannelHandlerContext ctx) {
        ...
        if (member.getState() != 0) {
            SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("您刚刚回到了基地请稍等才能进入战场"));
            return;
        }
        IAction action = (IAction)ParamTool.ACTION_MAP.get("changemap");
        action.action(ctx, GangBattleAction.GAOSHOU);
    }
    ...
    public void intoBattle(ChannelHandlerContext ctx) {
        ...
        IAction action = (IAction)ParamTool.ACTION_MAP.get("changemap");
        action.action(ctx, bangFight.iscamp(roleInfo.getGang_id()) ? GangBattleAction.ZHANC_Left : GangBattleAction.ZHANC_Right);
    }
    ...
    public void gohome(ChannelHandlerContext ctx) {
        ...
        if (bangFight == null || bangFight.BangState != 1) {
            IAction action = (IAction)ParamTool.ACTION_MAP.get("changemap");
            action.action(ctx, GangBattleAction.HOME_Left);
            return;
        }
        Member member = bangFight.getMember(roleInfo.getRolename(), roleInfo.getGang_id());
        if (member == null) {
            IAction action2 = (IAction)ParamTool.ACTION_MAP.get("changemap");
            action2.action(ctx, bangFight.iscamp(roleInfo.getGang_id()) ? GangBattleAction.HOME_Left : GangBattleAction.HOME_Right);
            return;
        }
        if (member.getState() != 0) {
            SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("您刚刚回到了基地请稍等才能进入战场"));
        ...
        IAction action3 = (IAction)ParamTool.ACTION_MAP.get("changemap");
        action3.action(ctx, bangFight.iscamp(roleInfo.getGang_id()) ? GangBattleAction.HOME_Left : GangBattleAction.HOME_Right);
    }
    ...
    public void ytz(ChannelHandlerContext ctx) {
        ...
        if (roleInfo.getTeam().split("\\|").length < GangBattleAction.MINSUM) {
            SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("最少人数" + GangBattleAction.MINSUM));
            return;
        }
        ...
        if (bangFight.PKstate != 0) {
            SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("#R当前未到挑战时间，#Y请根据系统提示#R发起挑战。"));
            return;
        }
        member.setState(3);
        ...
        buffer.append("#R发起了挑战，请帮派核心人员速速应战！");
    ...
    public void qxtz(ChannelHandlerContext ctx) {
        ...
        if (bangFight.Launch != null && bangFight.Launch.getKey().equals(roleInfo.getRolename())) {
            member.setState(0);
            bangFight.getzk(roleInfo.getRolename(), false);
    ...
    public void jstz(ChannelHandlerContext ctx) {
        ...
        if (bangFight.Launch == null) {
        ...
        else if (bangFight.Launch.getCamp().compareTo(roleInfo.getGang_id()) == 0) {
            ...
            buffer.append("#R");
            buffer.append(roleInfo.getRolename());
            buffer.append("#Y受不了#R ");
            buffer.append(launame);
            buffer.append("#Y的挑衅打了起来");
            bangFight.BattleNews(buffer.toString());
            ...
            LoginResult loginResult = (LoginResult)GameServer.getAllLoginRole().get(GameServer.getRoleNameMap().get(launame));
    ...
    public void qx(ChannelHandlerContext ctx, int v) {
        ...
        Member member = bangFight.getMember(roleInfo.getRolename(), roleInfo.getGang_id());
        Build build = bangFight.getBuild(v);
        if (build.getState() != Build.IDLE && build.getRoleName().equals(roleInfo.getRolename())) {
    ...
    public void bty(ChannelHandlerContext ctx) {
        ...
        Scene scene = SceneUtil.getScene(1001);
        if (scene == null) {
    ...
    public void TGDB(int i, ChannelHandlerContext ctx) {
        ...
        if (i >= 14 && i <= 16) {
            if (tgdbScene.isC(i - 13) == 0) {
                IAction action = (IAction)ParamTool.ACTION_MAP.get("changemap");
                if (i == 14) {
                    action.action(ctx, GangBattleAction.TGDB_2);
                }
                else if (i == 15) {
                    action.action(ctx, GangBattleAction.TGDB_3);
                }
                else if (i == 16) {
                    action.action(ctx, GangBattleAction.TGDB_4);
                }
            }
        }
        else if (i >= 17 && i <= 20 && tgdbScene.isC(i - 16) == 1) {
            FightingForesee fightingForesee = new FightingForesee();
            LoginResult roleInfo = (LoginResult)GameServer.getAllLoginRole().get(ctx);
            fightingForesee.setYidui(roleInfo.getTeam());
            fightingForesee.setType(1);
            fightingForesee.setI((int)tgdbScene.getTgdbMonsters()[i - 17].getBean().getI());
    ...
    public void ZZS(int i, ChannelHandlerContext ctx) {
        ...
        else if ((i == 23 || i == 24) && zzsScene.getI() == 2) {
    ...
    public void LTS(int value, int type, ChannelHandlerContext ctx) {
        ...
        if (value == 29) {
            ...
            if (!teams[0].equals(roleInfo.getRolename())) {
                return;
            }
            for (int i = 0; i < teams.length; ++i) {
                ...
                if ((int)login.getGrade() < 296) {
                    SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("最低2转100"));
                    return;
                }
            }
            IAction action = (IAction)ParamTool.ACTION_MAP.get("changemap");
            action.action(ctx, GangBattleAction.LTS);
            return;
        }
        else {
            if (value == 30) {
            ...
            if (arena.getPkMatch() == null) {
                SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("当前擂台无人摆擂"));
                return;
            }
            if (value == 26) {
                if (arena.getPkMatch().getPkMan1().getRoleId().compareTo(roleInfo.getRole_id()) == 0) {
                    SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("自己还想打自己?"));
                    return;
                }
                ...
                if (arena.getPkMatch().getPkMan1().getRoleId().compareTo(roleInfo.getRole_id()) != 0) {
                    SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("又不是你在摆擂"));
                    return;
                }
                ...
                if (arena.getBattleNumber() == 0) {
                    SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("还没有人前来挑战擂主"));
                    return;
                }
                if ((int)roleInfo.getFighting() != 0) {
                    return;
                }
                IAction action = (IAction)ParamTool.ACTION_MAP.get("battleconnection");
    ...
    public void RCFB(int value, ChannelHandlerContext ctx) {
        ...
        String msg = null;
        if (value == 31) {
            msg = rcScene.addNo1(roleInfo);
        }
        else if (value == 32) {
            msg = rcScene.addNo2(roleInfo);
        }
        if (msg != null) {
            SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement(msg));
        }
    ...
    public void PKLS(int value, ChannelHandlerContext ctx) {
        ...
        Scene scene = SceneUtil.getScene(1010);
        if (scene == null) {
            SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("活动已经关闭了"));
            return;
        }
    ...
    public void DNTG(int value, ChannelHandlerContext ctx) {
        ...
        if (value == 34) {
            ...
            dntgRole.setA(true);//大闹奖励
            Dorp dorp = GameServer.getDorp((dntgRole.getDNJF() <= 500000) ? "106" : "107");
            if (dorp != null) {
                double xs = (double)(1 + dntgRole.getDNJF() / 20000);
                if (xs >= 50.0) {
                    xs = 50.0;
                }
    ...
    public void FOLLOW(int value, int type, ChannelHandlerContext ctx) {
        ...
        if (type == 530) {
            ...
            if (bean == null || bean.getRobotType() != 4) {
                SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("你没有地煞星魂"));
                return;
            }
            ...
            Dorp dorp = GameServer.getDorp("300");//星魂奖励掉落
            if (dorp == null) {
                return;
            }
            for (int i = 0; i < teams.length; ++i) {
                ...
                if (login != null) {
                    DropUtil.getDrop(login, dorp.getDorpValue(), "地煞星魂礼包", 22, 1.0, null);
                }
            }
        }
        else if (type == 666) {
            MapMonsterBean bean = MonsterUtil.getFollowMonster(teams);
            if (bean == null || bean.getRobotType() != 66) {
                SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("你没有商旅"));
                return;
            }
            MonsterUtil.removeMonster2(bean);
            SendMessage.sendMessageToMapRoles(roleInfo.getMapid(), Agreement.getAgreement().battleStateAgreement("M" + bean.getI() + "^2"));
            Dorp dorp = GameServer.getDorp("666");
            if (dorp == null) {
                return;
            }
            for (int i = 0; i < teams.length; ++i) {
                ...
                if (login != null) {
                    DropUtil.getDrop(login, dorp.getDorpValue(), "商旅礼包", 22, 1.0, null);
                }
            }
        }
    ...
    public void SLDH(int value, ChannelHandlerContext ctx) {
        ...
        if (value == 39) {
            ...
            if (dorp != null) {
                sldhRole.setI(1);
                DropUtil.getDrop(roleInfo, dorp.getDorpValue(), (dc == 5) ? "水陆冠军礼包" : ((dc == 4) ? "水陆亚军礼包" : ((dc == 3) ? "水陆季军礼包" : ((dc == 2) ? "水陆8强礼包" : "水陆参与礼包"))), 22, 1.0, null);
            }
        }
    }
    ...
    public void GangNpc(int value, String ID, ChannelHandlerContext ctx) {
        ...
        if (value == 41 || value == 42 || value == 43) {
            if (domain.getGangGroup().getKj() <= 0) {
                SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("当前科技等级0级无法领取"));
                return;
            }
            if (roleInfo.getContribution().longValue() < 2000L) {
                SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("不足2000帮贡"));
                return;
            }
            ...
            if (roleData != null) {
                ...
                if (value == 41) {
                    ...
                    cardValue = "加强全系法术=" + (double)(domain.getGangGroup().getKj() + 1) * 1.5;
                }
                else if (value == 43) {
                    double add = (double)((domain.getGangGroup().getKj() + 1) * 2);
                ...
                roleInfo.setContribution(new BigDecimal(roleInfo.getContribution().longValue() - 2000L));
                assetUpdate.updata("B=-2000");
            ...
            if (roleInfo.getContribution().longValue() < 150L) {
            ...
            if (value == 44) {
                ...
                if (qm >= (long)Integer.parseInt(configure.getZhsqmsx())) {
                    SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("你的宝宝亲密已满"));
                    return;
                }
                if (!domain.useXY()) {
                ...
                qm = (long)((double)qm + 10000.0 * (1.0 + (double)(domain.getGangGroup().getXy() - 1) * 0.2));
                if (qm >= (long)Integer.parseInt(configure.getZhsqmsx())) {
                    qm = (long)Integer.parseInt(configure.getZhsqmsx());
                }
                pet.setFriendliness(Long.valueOf(qm));
                AllServiceUtil.getRoleSummoningService().updatePetRedis(pet);
                AssetUpdate assetUpdate = new AssetUpdate();
                assetUpdate.setType(AssetUpdate.USEGOOD);
                roleInfo.setContribution(new BigDecimal(roleInfo.getContribution().longValue() - 150L));
                assetUpdate.updata("B=-150");
                assetUpdate.setMsg("你的召唤兽#G" + pet.getSummoningname() + "#Y获得#G" + (int)(10000.0 * (1.0 + (double)(domain.getGangGroup().getXy() - 1) * 0.2)) + "#Y亲密");
                ...
                if (mount == null || mount.getRoleid().compareTo(roleInfo.getRole_id()) != 0) {
                    return;
                }
                if (value == 45) {
                    ...
                    roleInfo.setContribution(new BigDecimal(roleInfo.getContribution().longValue() - 150L));
                    assetUpdate2.updata("B=-150");
                    int addexp = (int)(500.0 * (1.0 + (double)(domain.getGangGroup().getXy() - 1) * 0.2));
                    ExpUtil.MountExp(mount, addexp);
                    AllServiceUtil.getMountService().updateMountRedis(mount);
                    assetUpdate2.setMsg("你的坐骑#G" + mount.getMountname() + "#Y获得#G" + (int)(500.0 * (1.0 + (double)(domain.getGangGroup().getXy() - 1) * 0.2)) + "#Y经验");
                    assetUpdate2.setMount(mount);
                    ...
                    if ((int)mount.getMountlvl() > 100) {
                        ...
                        Configure configure2 = (Configure)s2.get(Integer.valueOf(1));
                    ...
                    roleInfo.setContribution(new BigDecimal(roleInfo.getContribution().longValue() - 150L));
                    assetUpdate2.updata("B=-150");
                    int addvalue = (int)(200.0 * (1.0 + (double)(domain.getGangGroup().getXy() - 1) * 0.2));
                    int proficiency = (int)mount.getProficiency() + addvalue;
                    if (proficiency > up) {
                        proficiency = up;
                    }
                    mount.setProficiency(Integer.valueOf(proficiency));
                    AllServiceUtil.getMountService().updateMountRedis(mount);
                    assetUpdate2.setMsg("你的坐骑#G" + mount.getMountname() + "#Y获得#G" + (int)(200.0 * (1.0 + (double)(domain.getGangGroup().getXy() - 1) * 0.2)) + "#Y技能熟练度");
                    assetUpdate2.setMount(mount);
                    SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().assetAgreement(GsonUtil.getGsonUtil().getgson().toJson(assetUpdate2)));
                }
            }
        }
        else if (value == 47 || value == 48 || value == 49) {
            ...
            String name = null;
            if (value == 47) {
                lvl2 = domain.getGang().getGanggrade().intValue();
                xh = (int)(Math.pow(2.0, (double)lvl2) * 5000.0);
                name = "帮派等级";
            }
            else if (value == 48) {
                lvl2 = domain.getGangGroup().getKj();
                xh = (int)(Math.pow(2.0, (double)lvl2) * 2000.0);
                name = "科技等级";
            }
            else if (value == 49) {
                lvl2 = domain.getGangGroup().getXy();
                xh = (int)(Math.pow(2.0, (double)lvl2) * 2000.0);
                name = "驯养师等级";
            }
            if (lvl2 >= 5) {
            ...
            if (value != 47 && lvl2 >= domain.getGang().getGanggrade().intValue()) {
                SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("不能超过帮派等级"));
                return;
            }
            if (domain.getGang().getBuilder().longValue() < (long)xh) {
    ...
    static {
        GangBattleAction.MINSUM = 3;
        GangBattleAction.HOME_Left = "";
        GangBattleAction.HOME_Right = "";
        GangBattleAction.OUTBANG = "";
        GangBattleAction.GAOSHOU = "";
        GangBattleAction.ZHANC_Left = "";
        GangBattleAction.ZHANC_Right = "";
        GangBattleAction.TGDB_2 = "";
        GangBattleAction.TGDB_3 = "";
        GangBattleAction.TGDB_4 = "";
        GangBattleAction.OUTCA = "";
        GangBattleAction.LTS = "";
        GangBattleAction.WSS = "";
        ChangeMapBean change = new ChangeMapBean();
        change.setMapid("3315");
        change.setMapx(276);
        change.setMapy(1703);
        GangBattleAction.HOME_Left = GsonUtil.getGsonUtil().getgson().toJson(change);
        change.setMapx(2122);
        change.setMapy(367);
        GangBattleAction.HOME_Right = GsonUtil.getGsonUtil().getgson().toJson(change);
        change.setMapx(2309);
        change.setMapy(1687);
        GangBattleAction.GAOSHOU = GsonUtil.getGsonUtil().getgson().toJson(change);
        change.setMapx(345);
        change.setMapy(1422);
        GangBattleAction.ZHANC_Left = GsonUtil.getGsonUtil().getgson().toJson(change);
        change.setMapx(2085);
        change.setMapy(616);
        GangBattleAction.ZHANC_Right = GsonUtil.getGsonUtil().getgson().toJson(change);
        change.setMapid("1207");
        change.setMapx(7920);
        change.setMapy(4340);
        GangBattleAction.OUTBANG = GsonUtil.getGsonUtil().getgson().toJson(change);
        change.setMapid("3326");
        change.setMapx(1200);
        change.setMapy(600);
        GangBattleAction.TGDB_2 = GsonUtil.getGsonUtil().getgson().toJson(change);
        change.setMapid("3327");
        change.setMapx(1400);
        change.setMapy(1200);
        GangBattleAction.TGDB_3 = GsonUtil.getGsonUtil().getgson().toJson(change);
        change.setMapid("3328");
        change.setMapx(1200);
        change.setMapy(900);
        GangBattleAction.TGDB_4 = GsonUtil.getGsonUtil().getgson().toJson(change);
        change.setMapid("1207");
        change.setMapx(4294);
        change.setMapy(2887);
        GangBattleAction.OUTCA = GsonUtil.getGsonUtil().getgson().toJson(change);
        change.setMapid("3333");
        change.setMapx(1040);
        change.setMapy(840);
        GangBattleAction.LTS = GsonUtil.getGsonUtil().getgson().toJson(change);
        change.setMapid("1732");
        change.setMapx(660);
        change.setMapy(2600);
        GangBattleAction.WSS = GsonUtil.getGsonUtil().getgson().toJson(change);
    }
}
