...
public class FightbattleConnectionAction implements IAction
{
    ...
    public void action(ChannelHandlerContext ctx, String message) {
        ...
        if (match != null) {
            SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("携带战书没法进入观战"));
            return;
        }
        ...
        if (fight == -1) {
            BattleData battleData = (BattleData)BattleThreadPool.BattleDatas.get(loginResult.getFighting());
            ...
            for (int i = 0; i < teams.length; ++i) {
                if (battleData != null) {
                    battleData.getParticipantlist().remove(teams[i]);
                }
