...
public class PayDayGradeSureAction implements IAction
{
    ...
    public void action(ChannelHandlerContext ctx, String message) {
        ...
        List<ChongjipackBean> chongList = (List<ChongjipackBean>)GameServer.getPackgradecontrol().get(Integer.valueOf(4));
        ChongjipackBean chongListBean = null;
        int condition = VipRewardUtils.everydayRecharge(message);
        BigDecimal daypaysum = roleInfo.getDaypaysum();
        int i = 0;
        while (i < chongList.size()) {
            ...
            if (chongziGrade == (int)((ChongjipackBean)chongList.get(i)).getPackgradetype()) {
                ...
                if (daypaysum.doubleValue() < (double)Double.valueOf((double)(int)((ChongjipackBean)chongList.get(i)).getCanpaymoney())) {
